...
import getMinMaxTemperature from "../utilities/getMinMaxTemperature";

async function setWeatherData(data) {
  try {
    const todayTemperature = document.querySelector('.weather_today--temperature');
    const todayTemperatureAllDay = document.querySelector('.weather_today--temperature_all_day');
    ...
    const wearheAllWeek = document.querySelector('.weather_future--all_week');
    ...
    const todayMinMaxTemperature = await getMinMaxTemperature(weather);
    ...
    todayTemperatureAllDay.textContent = `${todayMinMaxTemperature.min}${String.fromCharCode(176)} / ${todayMinMaxTemperature.max}${String.fromCharCode(176)}`;
    todayTemperatureAllDay.setAttribute('temperature-low', `${todayMinMaxTemperature.min}`);
    todayTemperatureAllDay.setAttribute('temperature-high', `${Math.trunc(todayMinMaxTemperature.max)}`);
    ...
    todayVisibility.textContent = `${translateUI()[15]}: ${Math.trunc(weather.currently.visibility)} ${translateUI()[17]}`;
    todayHectopascals.textContent = `${translateUI()[18]}: ${convertPressure(weather.currently.pressure)} ${translateUI()[19]}`;
    wearheAllWeek.textContent = `${translateUI()[21]} ${weather.daily.summary.toLowerCase()}`;
